...
int main(int argc, char *argv[] )
{
  ...
  else if (taskid == 1 || taskid == 2|| taskid == 3 || taskid == 4|| taskid == 5|| taskid == 6|| taskid == 7)
  {
  my_size = (n/4)+0+2;
  input_temp = new double*[my_size];
  output_temp = new double*[my_size];
 ...
 while ( iter <niters )
  {
    ...
    if(taskid==0)
     ...
     else if (taskid==3)
        {
         ...
         MPI_Recv(input_temp[my_size-2], temp_size, MPI_DOUBLE, 4, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         MPI_Send(input_temp[my_size-1], temp_size, MPI_DOUBLE, 4, 0,MPI_COMM_WORLD);
  
         }
     else if (taskid==4)
        {
         MPI_Send(input_temp[my_size-1], temp_size,MPI_DOUBLE, 5, 0,MPI_COMM_WORLD);
         MPI_Recv(input_temp[my_size-2], temp_size, MPI_DOUBLE, 5, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
 
         MPI_Send(input_temp[1], temp_size, MPI_DOUBLE, 3, 0,MPI_COMM_WORLD);
         MPI_Recv(input_temp[0], temp_size, MPI_DOUBLE, 3, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  
         }
     else if (taskid==5)
        {
         MPI_Recv(input_temp[0], temp_size, MPI_DOUBLE, 4, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         MPI_Send(input_temp[1], temp_size,MPI_DOUBLE, 4, 0,MPI_COMM_WORLD);
 
         MPI_Recv(input_temp[my_size-2], temp_size, MPI_DOUBLE, 6, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         MPI_Send(input_temp[my_size-1], temp_size, MPI_DOUBLE, 6, 0,MPI_COMM_WORLD);
  
         }
     else if (taskid==6)
        {
         MPI_Recv(input_temp[my_size-2], temp_size, MPI_DOUBLE, 7, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         MPI_Send(input_temp[my_size-1], temp_size,MPI_DOUBLE, 7, 0,MPI_COMM_WORLD);
 
         MPI_Send(input_temp[1], temp_size, MPI_DOUBLE, 5, 0,MPI_COMM_WORLD);
         MPI_Recv(input_temp[0], temp_size, MPI_DOUBLE, 5, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  
         }
     else if (taskid==7)
        {
         MPI_Send(input_temp[1], temp_size,MPI_DOUBLE, 6, 0,MPI_COMM_WORLD);
         MPI_Recv(input_temp[0], temp_size, MPI_DOUBLE, 6, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
 
         MPI_Send(input_temp[my_size-1], temp_size, MPI_DOUBLE, 8, 0,MPI_COMM_WORLD);
         MPI_Recv(input_temp[my_size-2], temp_size, MPI_DOUBLE, 8, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  
         }
     else if(taskid==8) 
        {
         MPI_Recv(input_temp[0], temp_size, MPI_DOUBLE, 7, 0,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         MPI_Send(input_temp[1], temp_size, MPI_DOUBLE, 7, 0,MPI_COMM_WORLD);
